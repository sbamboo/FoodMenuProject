...
<h2 id="sending_a_request">Sending a request</h2>
This api takes in requests using GET url-parmeters and responds with data in the JSON format.<br><br>

<h3 id="api_response">Api Response</h3>
The data returned by `v1` of the api follows this schema:
```json
{
    "format": 0,
...
<h3 id="requesting_by_date">Requesting by Date</h3>
...
<h3 id="requesting_by_week">Requesting by Week (Current Year)</h3>
...
<h3 id="requesting_by_year_and_week">Requesting by Year and Week</h3>
...
<h3 id="requesting_by_year">Requesting by Year (Full Year's Data)</h3>
...
All the api endpoints above support the addition of optional parameters to filter the output.

<br>

`?excludeWeekends`/`&excludeWeekends` can be added to exclude all saturdays and sundays for all the requested weeks.

For example a request could be:
```api.request
{url}/v1?year=2025&excludeWeekends
```
<br>

`?excludeHolidays`/`&excludeHolidays` can be added to exclude all Swedish nation holidays. (See [#Excluded Holidays](#excluded_holidays))

For example a request could be:
```api.request
{url}/v1?year=2025&excludeHolidays
```
<br>

`?day=<dayIndex>`/`&day=<dayIndex>` can be added to select only this day index in each of the requested weeks. This follows the same (1=Monday to 7=Sunday) indexing.

If the requested day does not exist the requested weeks will still be filtered and thus end up empty.

For example a request could be:
```api.request
{url}/v1?year=2025&day=5
```
<br>

<h3 id="excluded_holidays">Excluded Holidays</h3>
<br>
