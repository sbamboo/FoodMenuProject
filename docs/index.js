const INITIAL_COPY_SVG = `<svg width="100%" height="100%" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const CLICKED_COPY_SVG = `<svg width="100%" height="100%" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
window.onload = function () {
    document.querySelectorAll('.codeblock_wcopy').forEach(function (codeBlock) {
        ...
        // Create a copy button with the initial SVG
        const button = document.createElement('button');
        button.setAttribute('title', 'Copy to clipboard');
        button.setAttribute('alt', 'Copy to clipboard');
        
        // SVG for the initial copy button
        button.innerHTML = INITIAL_COPY_SVG;
        button.classList.add('copy-btn');
    
        // Attach click event to copy the text to the clipboard
        button.addEventListener('click', function () {
            // Change button SVG to "clicked" version
            button.innerHTML = CLICKED_COPY_SVG;

            // Copy the text to the clipboard
            navigator.clipboard.writeText(textContent)
                .then(() => {
                    // Revert back to the original SVG after 1 second
                    setTimeout(() => {
                        button.innerHTML = INITIAL_COPY_SVG;
                    }, 1000);
                })
        ...
        // Add the button after the pre element
